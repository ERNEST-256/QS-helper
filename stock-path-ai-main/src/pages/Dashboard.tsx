...
import { TrendingUp, User, LogOut, DollarSign, PieChart as PieChartIcon, Brain } from "lucide-react";
import { useNavigate } from "react-router-dom";

const Dashboard = () => {
  const [stockName, setStockName] = useState("");
  const [allocation, setAllocation] = useState("");
  const [investments, setInvestments] = useState<Array<{ name: string; amount: number; percentage: number }>>([]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const totalAmount = updatedInvestments.reduce((sum, inv) => sum + inv.amount, 0);
  ...
  const aiExplanation =
    investments.length > 0
      ? `Based on your investment preferences, I've analyzed your portfolio allocation across ${investments.length} stock(s). The largest allocation is ${investments[0]?.percentage.toFixed(
          1
        )}% to ${investments[0]?.name}. This diversification strategy helps balance risk and potential returns. Consider monitoring market trends and adjusting allocations based on performance metrics.`
      : "";
      ...
      <header className="border-b bg-card shadow-soft">
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold">StockPath AI</h1>
          </div>
          <DropdownMenu>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      {investments.map((investment, index) => (
                        <div key={index} className="flex justify-between items-center p-3 rounded-lg bg-muted">
                          <div>
                          ...
                          <p className="font-bold text-primary">${investment.amount.toFixed(2)}</p>
                        </div>
                      ))}
                    </div>
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          <Tooltip formatter={(value) => [`${typeof value === "number" ? value.toFixed(1) : value}%`, "Allocation"]} />
                        </PieChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
